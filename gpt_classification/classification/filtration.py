    ...
    async def __if_garbage(self, _dict):
        try:
            response = await g4f.ChatCompletion.create_async(
                ...
                messages=[{"role": "user", "content": f'{files.get_promt().split("<text>")[0]}{_dict["Text"]}{files.get_promt().split("<text>")[1]}'}]
            )
            ans = ''
